...
// Layer: CLI service
// Exports: startBridge
// Depends on: ws, crypto, os, path, ./codex-home, ./qr, ./codex-desktop-refresher, ./codex-transport, ./rollout-watch, ./voice-handler, ./ios-app-compatibility
...
const { createShortPairingCode, SHORT_PAIRING_CODE_LENGTH, printQR } = require("./qr");
  ...
  function handleBridgeManagedThreadTurnsListRequest(rawMessage) {
    ...
    (async () => {
      try {
        ...
        sendApplicationResponse(sanitizeThreadHistoryImagesForRelay(
...
function parseAdaptiveThreadTurnsListRequest(rawMessage) {
  ...
  if (parsed.method !== "thread/turns/list" || parsed.id == null) {
    return null;
  }
...
async function fetchAdaptiveThreadTurnsListForRelay(request, {
  ...
  while (combinedTurns.length < requestedLimit) {
    ...
    turnsKey = turnsKey || pageTurnsKey;
    firstResult = firstResult || pageResult;
    lastResult = pageResult;
    combinedTurns = combinedTurns.concat(pageResult[pageTurnsKey]);
    response = {
      id: request.id,
    ...
    if (combinedTurns.length >= requestedLimit || !hasRelayCursor(nextCursor) || pageResult[pageTurnsKey].length === 0) {
      break;
    }
...
function sanitizeThreadTurnsListForRelay(rawMessage) {
  ...
  const turnsKey = RELAY_TURNS_LIST_RESULT_KEYS.find((key) => Array.isArray(result[key]));
  if (!turnsKey) {
    return rawMessage;
  }

  let didSanitize = false;
  const sanitizedTurns = result[turnsKey].map((turn) => {
    const sanitizedTurn = sanitizeRelayHistoryTurn(turn);
...
function sanitizeRelayHistoryTurn(turn) {
  if (!turn || typeof turn !== "object" || !Array.isArray(turn.items)) {
    return turn;
  }

  let turnDidChange = false;
  const turnThreadId = normalizeNonEmptyString(turn.threadId)
    || normalizeNonEmptyString(turn.thread_id);
  const sanitizedItems = turn.items.map((item) => {
    const sanitizedItem = sanitizeRelayHistoryItem(item, turnThreadId);
...
function sanitizeRelayHistoryItem(item, threadId = "") {
  if (!item || typeof item !== "object") {
    return item;
  }
